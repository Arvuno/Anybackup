...
def upgrade() -> None:
    ...
    with op.batch_alter_table("t_conversation_writeback_idempotency") as batch_op:
        batch_op.drop_column("f_stream_id")


def downgrade() -> None:
    with op.batch_alter_table("t_conversation_writeback_idempotency") as batch_op:
        batch_op.add_column(sa.Column("f_stream_id", sa.String(length=128), nullable=True))
