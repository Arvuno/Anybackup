...
- Third-party open-source components shipped with this chart (the Keycloak container image, etc.) are declared uniformly in [`THIRD_PARTY_NOTICES.md`](./THIRD_PARTY_NOTICES.md).
- When upgrading the Keycloak version or replacing the image, the Inventory table of the above notice file MUST be updated in the same commit.
- The chart only reaches an external PostgreSQL via JDBC, which is a pure protocol-level invocation and does not trigger any third-party open-source component notice obligation.
