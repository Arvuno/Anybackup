...
func newBackupConfigDetailCommand() meta.CommandMeta {
	return meta.CommandMeta{
		Domain:        "vmware",
		CanonicalPath: []string{"vmware", "backup-config", "detail"},
		Use:           "detail",
		Description:   "Get VMware backup config detail",
		Risk:          meta.RiskReadOnly,
		AuthType:      meta.AuthAKSK,
		ReadOnly:      true,
		BindFlags: func(cmd *cobra.Command, rt *meta.Runtime) {
			rt.BindStringFlag(cmd.Flags(), "object-id", "", "VMware object ID")
		},
		...
		BuildRequest: func(rt *meta.Runtime) (console.RequestSpec, error) {
			return console.RequestSpec{
				...
				Path:     fmt.Sprintf("/backupmgm/v1/virtual/vmware/%s/backup_config", rt.String("object-id")),
				ReadOnly: true,
			}, nil
		},
	}
