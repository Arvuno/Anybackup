...
func newBackupConfigGetCommand() meta.CommandMeta {
	return meta.CommandMeta{
		Domain:        "protect",
		CanonicalPath: []string{"protect", "backup-config", "get"},
		Use:           "get",
		Description:   "Get backup destination config by object ID",
		...
		BuildRequest: func(rt *meta.Runtime) (console.RequestSpec, error) {
			return console.RequestSpec{
				...
				Path:     fmt.Sprintf("/backupmgm/v2/protect_object/%s/backup_config", rt.String("object-id")),
				ReadOnly: true,
			}, nil
		},
	}
