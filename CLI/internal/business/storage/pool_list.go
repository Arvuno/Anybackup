...
func newPoolListCommand() meta.CommandMeta {
	return meta.CommandMeta{
		Domain:        "storage",
		CanonicalPath: []string{"storage", "pool", "list"},
		Use:           "list",
		...
		BuildRequest: func(rt *meta.Runtime) (console.RequestSpec, error) {
			...
			return console.RequestSpec{
				...
				Path:     fmt.Sprintf("/storageresmgm/v1/%s/pool?%s", rt.String("storage-svc-id"), query),
