...
func newBackupConfigDetailCommand() meta.CommandMeta {
	return meta.CommandMeta{
		Domain:        "host",
		CanonicalPath: []string{"host", "backup-config", "detail"},
		Use:           "detail",
		Description:   "Get host backup config detail",
		...
		BuildRequest: func(rt *meta.Runtime) (console.RequestSpec, error) {
			return console.RequestSpec{
				...
				Path:     fmt.Sprintf("/backupmgm/v1/file/backup_config/%s", rt.String("object-id")),
				ReadOnly: true,
			}, nil
		},
	}
