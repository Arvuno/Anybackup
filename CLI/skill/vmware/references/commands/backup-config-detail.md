# `foundation-cli vmware backup-config detail`

## Command Summary

| Item | Value |
|---|---|
| CLI | `foundation-cli vmware backup-config detail --tenant-id <tenant-id> --object-id <object-id>` |
| Method | `GET` |
...
| changeBlockTracking | bool | 是否开启 CBT |
...
- 已按 [backup_config_detail.go](/e:/go/src/foundation-cli/internal/business/vmware/backup_config_detail.go) 与 [backup_config_create.go](/e:/go/src/foundation-cli/internal/business/vmware/backup_config_create.go) 校验返回字段。
...
| 项 | 值 |
|---|---|
| CLI | foundation-cli\ vmware\ backup-config\ detail |
| 风险 | read-only |
