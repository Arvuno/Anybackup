# `foundation-cli network subnet node ip remove`

## 命令摘要

| 项目 | 值 |
|---|---|
| CLI | `foundation-cli network subnet node ip remove --tenant-id <tenant-id> --storage-svc-id <storage-svc-id> --subnet-id <subnet-id> --node-id <node-id> --ip <ip>` |
| 方法 | `DELETE` |
| 路径 | `/clusters/v1/{storageSvcId}/subnet/nodes/{subnetId}/{nodeId}` |
| 风险 | `写入` |
...
| `--subnet-id` | 路径 + Body | `{subnetId}` + `subnetId` |
| `--node-id` | 路径 + Body | `{nodeId}` + `nodeId` |
| `--ip` | Body | `ip` |
...
1. CLI 会同时构造路径参数和 body 参数。
2. `subnetId`、`nodeId` 不会因为已经出现在路径中就从 body 里省略。
3. 本命令不使用 `--data` 或 `--body-file`。
...
1. 删除前建议先执行 `network subnet node ip list`，确认目标 IP 当前已绑定在对应节点上。
2. 该接口协议要求路径和 body 都带目标信息，agent 不应自行简化。
3. 这是写操作，适合在自动化流程里增加显式确认步骤。
