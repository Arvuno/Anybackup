...
| runners[].dbParam | object[] | 否 | 数据库Runner专属配置，仅当runnerType为MySQL/Oracle等数据库类型时填写；其中user（数据库用户名）、dbPath（数据库路径/实例/端口等信息）必须由用户选择确认后输入，禁止Agent自动生成或猜测 |

#### 部署阶段请求体示例（MySQL Runner部署）
```json
{
  "jobName": "ceshimysql",
  "LinuxInstallPath": "/opt",
  "WindowsInstallPath": "C:\\BackupAgent",
  "concurrent": 1,
  "uninstallType": false,
  "hostList": ["98597ed241dd11f19ff300163e33df73"],
  "runners": [
    {
      "runnerType": "MySQL",
      "dbParam": [
        {
          "user": "tt", // 必须由用户选择确认的数据库用户名
          "dbPath": "321313" // 必须由用户选择确认的数据库路径/实例标识
        }
      ]
    }
  ]
...
```
> ⚠️ 强制规则：数据库类型Runner部署时，`dbParam`下的`user`、`dbPath`属于用户专属配置，禁止Agent自动生成或猜测，必须明确让用户选择/输入后再构造请求。
...
foundation-cli client deploy \
  ...
  --sk <sk> \
  --os linux \
  --data '{"name":"deploy-job","hostList":["115.190.186.186"],"port":22,"administrator":true,"account":{"rootPassword":"12345678"},"runners":[{"runnerType":"MySQL","dbParam":[{"user":"tt","dbPath":"321313"}]}]}'
```
